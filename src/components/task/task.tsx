...
const Task = ({msg, completed, index, toggleCompleted, deleteTsk}:{msg:string, completed:boolean ,index:number, toggleCompleted: (bool:boolean, index:number) => void, deleteTsk: (n:number) => void}) => {
    const id = useId();

    return (<div className="task border-b border-border p-5 flex"> 
        <div className="round relative mr-10">
            <input defaultChecked={completed} onClick={() => toggleCompleted(!completed, index)} type="checkbox" className="invisible" id={id} />
        ...
        <input disabled value={completed+msg} className="bg-transparent dark:text-white text-slate-950 w-full cursor-pointer" type="text"/>
